  ...
  <div class="row">
    <div class="col-sm-12 fontsizebox">
      ...
      <p class="lead text-justify">
        {$tiger_reason.reason_content}
      </p> 
      <!-----------遺址發掘原因文章置放處 結束(內容放置於</p>前)------------->
      {if $isAdmin}
        <a href="admin.php?op=tiger_reason_form&reason_sn={$tiger_reason.reason_sn}" class="btn btn-warning">編輯文章</a>
        <a href="admin.php?op=delete_tiger_reason&reason_sn={$tiger_reason.reason_sn}" class="btn btn-danger">刪除文章</a>
      {/if}
    </div>
  </div>
</div>
