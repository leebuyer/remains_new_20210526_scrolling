...
<div class="container mt-3">
  ...
  <button type="button" class="close close-alert" data-dismiss="modal">&times;</button>
    ...
    <div class="row">
      <div class="col-sm-6">
        <img src="{$tree_more.pic}" alt="{$tree_more.more_title}" class="img-thumbnail img-fluid zoom">  
      </div>
      <div class="col-sm-6">
        <div class="alert alert-secondary">
          <h3 class="fontsizebox">{$tree_more.more_title}</h3>
        </div>
        <div class="alert alert-secondary">
          <h3 class="text-justify fontsizebox">{$tree_more.more_content}</h3>
        </div>
        <div>
        {if $isAdmin}
          <a href="admin.php?op=tree_more_form&more_sn={$tree_more.more_sn}" class="btn btn-warning">編輯文章</a>
          <a href="admin.php?op=delete_tree_more&more_sn={$tree_more.more_sn}" class="btn btn-danger">刪除文章</a>
        {/if}
      </div>
      </div>
    </div>
