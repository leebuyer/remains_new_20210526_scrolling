  ...
  <div class="row">
    ...
    <!-----------一起考古去文章置放處 開始(內容放置於<p>後)------------->
      <p class="lead text-justify">
        {$tree_action.action_content}
      </p>
      <!-----------一起考古去文章置放處 結束(內容放置於</p>前)------------->
      {if $isAdmin}
          <a href="admin.php?op=tree_action_form&action_sn={$tree_action.action_sn}" class="btn btn-warning">編輯文章</a>
          <a href="admin.php?op=delete_tree_action&action_sn={$tree_action.action_sn}" class="btn btn-danger">刪除文章</a>
        {/if}
    </div>
  </div>
</div>
